...
def main():
    ...
    # Initialize best model saver, metric calculator with class metrics, and early stopping
    ...
    # Create CSV file for per-class metrics
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Extract per-class precision and recall properly
        # The precision/recall tensors have shape: [T, R, K, A, M] where:
        # T = IoU thresholds (1), R = recall thresholds (101), K = classes, 
        # A = area ranges (4), M = max detections (3)
        ...
        try:
            ...
            precision_tensor = metric_summary['precision']  # Shape: [1, 101, K, 4, 3]
            recall_tensor = metric_summary['recall']  # Shape: [1, K, 4, 3]
            
            # Debug: print shapes
            print(f"Debug - Precision shape: {precision_tensor.shape}, Recall shape: {recall_tensor.shape}")
            ...
            if precision_tensor.shape[2] > 1:
                ...
                # Extract overall precision/recall for all classes combined
        ...
        # Calculate overall precision and recall (average across classes, excluding background)
        ...
        print(f"Epoch #{epoch+1} Precision: {overall_precision:.3f}")   
        print(f"Epoch #{epoch+1} Recall: {overall_recall:.3f}")
        
        # Print per-class metrics
        for i, class_name in enumerate(CLASSES[1:]):  # Skip background
            if i < len(precision_per_class):
                print(f"  {class_name} - P: {precision_per_class[i]:.3f}, R: {recall_per_class[i]:.3f}")
        ...
        if map_per_class.dim() > 0 and map_per_class.numel() > 1:
            class_maps = [map_per_class[i].item() for i in range(len(CLASSES))]
        else:
            class_maps = [map_per_class.item()] * len(CLASSES)
        
        # Save metrics for each class (skip background)
        for i, class_name in enumerate(CLASSES[1:], start=1):
            epoch_metrics.append({
                "epoch": epoch + 1,
                "class_name": class_name,
                "map50": class_maps[i] if i < len(class_maps) else 0.0,
                "precision": precision_per_class[i-1] if (i-1) < len(precision_per_class) else 0.0,
                "recall": recall_per_class[i-1] if (i-1) < len(recall_per_class) else 0.0,
                "lr": current_lr,
                "loss": train_loss
            })
        
        pd.DataFrame(epoch_metrics).to_csv(csv_path, mode='a', index=False, header=False)
    ...
    if train_loss_list and val_loss_list and map_list:
        ...
        print(f"Final Precision: {precision_scores[-1]:.4f}")
        print(f"Final Recall: {recall_scores[-1]:.4f}")
